...
class RestartAndRollbackValidationTests(unittest.TestCase):
    def test_restart_requires_exact_confirmation(self) -> None:
        with self.assertRaises(pending.ValidationError):
            pending.build_restart_request({"confirmation": "restart"})
        result = pending.build_restart_request({"confirmation": "RESTART"})
        self.assertEqual(result["schema_version"], 1)
    ...
    def test_reason_is_bounded(self) -> None:
        with self.assertRaises(pending.ValidationError):
            pending.build_restart_request(
                {"confirmation": "RESTART", "reason": "x" * (pending.MAX_REASON_CHARACTERS + 1)}
            )

    def test_rejects_unsupported_field(self) -> None:
        with self.assertRaises(pending.ValidationError):
            pending.build_restart_request({"confirmation": "RESTART", "extra": 1})
...
class DirectoryTransportTests(unittest.TestCase):
    ...
    def test_write_then_read_pending_request(self) -> None:
        request = pending.build_restart_request({"confirmation": "RESTART"})
        pending.write_pending_request(self.directory, pending.RESTART_REQUEST_FILE, request)
        read_back = pending.read_pending_request(self.directory, pending.RESTART_REQUEST_FILE)
    ...
    def test_read_pending_request_missing_file_returns_none(self) -> None:
        self.assertIsNone(pending.read_pending_request(self.directory, pending.RESTART_REQUEST_FILE))

    def test_read_result_missing_file_returns_none(self) -> None:
        self.assertIsNone(pending.read_result(self.directory, pending.RESTART_RESULT_FILE))

    def test_read_result_returns_bounded_result(self) -> None:
        result_path = self.directory / pending.RESTART_RESULT_FILE
        result_path.write_text('{"outcome": "success"}', encoding="utf-8")
        self.assertEqual(pending.read_result(self.directory, pending.RESTART_RESULT_FILE), {"outcome": "success"})
