...
- [x] Scoped, confirmed restart-only action.
...
| `restart-request` | full-stack | in-progress | Confirmation-word gate, CSRF/origin/session enforcement, and audit recording covered by backend tests; frontend `RestartAction` implemented. Root `valheim-restart-request` helper implemented with a 15-minute cooldown and reused log-tail health check, verified via sandboxed smoke tests; not installed/enabled — restart-capable helpers require explicit authorization before being triggered on the live host. |
