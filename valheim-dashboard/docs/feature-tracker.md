...
| `pending-manifest` | full-stack | in-progress | The editor seeds from the declared manifest (or a valid queued request) and submits the complete result after explicit additions, uninstall requests, or active-version selection. Version lists come from a bounded exact-package lookup in the same fixed Hexium catalog; resolver-managed dependencies remain read-only. It never presents an empty draft when the authoritative manifest is unavailable. Fixed-schema validation, CSRF/origin/session enforcement, duplicate-request conflict handling, and audit recording are covered by backend tests; frontend lint/build and modpack snapshot tests pass. Not yet deployed or live-verified. |
| `mod-config-editor` | full-stack | in-progress | The root-owned status publisher exports a bounded, separately validated view of existing managed BepInEx `.cfg` files. The dashboard edits only existing section/key values and queues them through the existing CSRF-protected, audited, serialized helper boundary. The root helper independently validates and atomically writes persistent overrides, preserving comments and unknown lines for the next scheduled maintenance staging run. Dashboard and modpack unit tests plus frontend lint/build pass; not yet deployed or live-verified. |
...
| `hexium-package-search` | in-progress | Fixed-host, redirect-rejecting, bounded catalogue search caches the Hexium v1 package list for 15 minutes and returns at most 10 allow-listed matches. The UI accepts a package name or an allow-listed Hexium `/mods/<namespace>/<package>` link, renders matches, and requires an explicit add or latest-version update before it changes the current manifest draft. Raw package/version/channel/role entry has been removed. The incremental-draft source changes are validated by frontend lint/build and must be deployed before live verification. |
