...
It also seeds the blank-webhook template
`/opt/valheim/modpack/config-overrides/io.hexium.valheim.discordnotifier.cfg`
when absent, or preserves an existing active notifier config when one was
already generated by BepInEx. This makes the Valheim Discord Notifier settings
available in the dashboard before the next maintenance run without committing a
webhook credential. Set `Discord.WebhookUrl` only on the server or through the
trusted dashboard; an existing override is never overwritten by deployment.
